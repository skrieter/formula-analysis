...
import org.spldev.util.io.binary.BinaryFormat;
...
public class BinarySampleFormat extends BinaryFormat<SolutionList> {
	...
	public Result<SolutionList> parse(Input source) {
		...
		try {
			int numberOfVariables = readInt(inputStream);
			List<String> variableNames = new ArrayList<>(numberOfVariables);
			for (int i = 0; i < numberOfVariables; i++) {
				variableNames.add(readString(inputStream));
			}
			VariableMap variableMap = new VariableMap(variableNames);
			int numberOfSolutions = readInt(inputStream);
			List<LiteralList> solutionList = new ArrayList<>(numberOfSolutions);
